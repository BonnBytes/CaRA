...
class CifarData(base.ImageTfdsData):
  ...
  def __init__(self, num_classes=10, data_dir=None, train_split_percent=None):
    ...
    dataset_name = f"cifar{num_classes}:3.*.*"
    name = dataset_name.split(':')[0]
    dataset_builder = tfds.builder(dataset_name, data_dir=f'../data/{name}/tfrecord')
    dataset_builder.download_and_prepare(
      download_dir=f'../data/{name}/raw',
      download_config=tfds.download.DownloadConfig(
          extract_dir=f'../data/{name}/extracted',
